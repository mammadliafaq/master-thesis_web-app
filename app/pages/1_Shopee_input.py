...
from app.utils.eval_utils import (generate_image_features,
                                  generate_text_features,
                                  get_concatenated_predictions)
...
def get_matches(valid, concatenated_features, config):
    f1_score, output_df = get_concatenated_predictions(
        valid, concatenated_features, threshold=config.concatenated_threshold
    )
    return output_df


def main(args: argparse.Namespace):
    st.title("Select the index of the item from the Shopee dataset")
